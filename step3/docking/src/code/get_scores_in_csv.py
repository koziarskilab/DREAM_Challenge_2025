...
def main():
    for dock_res_pro_dir in docking_res_protein_list:
        ...
        docked_ligands_list = [
            dock_ligand
            for dock_ligand in dock_res_pro_dir.iterdir()
            if dock_ligand.suffix == ".sdf"
        ]
        ...
        if not required_columns.issubset(df.columns):
            print(
                f"[ERROR] Please check CSV file. It lacks following required column(s): {required_columns - set(df.columns)}"
            )
            sys.exit(1)
        ...
        for index, row in df.iterrows():
            if (
                pd.isna(row["BEST_SCORE"])
                and pd.isna(row["VINA_SCORE"])
                and pd.isna(row["SHAPE_SCORE"])
                and pd.isna(row["ESP_SCORE"])
            ):
                mol_name = row["MOL_ID"]

                if mol_name in results_dict:
                    df.at[index, "VINA_SCORE"] = results_dict[mol_name]["VINA_SCORE"]
                    df.at[index, "BEST_SCORE"] = results_dict.get(mol_name)[
                        "BEST_SCORE"
                    ]
                    df.at[index, "SHAPE_SCORE"] = results_dict.get(mol_name)[
                        "SHAPE_SCORE"
                    ]
...
if __name__ == "__main__":
    ...
    docking_res_protein_list = [
        protein_dir for protein_dir in docking_results.iterdir() if protein_dir.is_dir()
    ]
