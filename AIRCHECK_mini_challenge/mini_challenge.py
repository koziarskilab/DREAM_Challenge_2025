...
import os
from helper import Dataset, ProcessData, SimplifiedDrugFilters


df_train = Dataset(
    "../datasets/DEARM/AIRCHECK_mini_challenge/TrainDataset_Aircheck.csv"
).get_dataframe()
# Define the ratio of negative samples (N times more than positives)
N = 2  # The number of negative samples to select for each positive sample
...
print("Number of binders in the training set:", (df_train["DELLabel"] == 1).sum())
print("Number of non-binders in the training set:", (df_train["DELLabel"] == 0).sum())

DevData_path = "../datasets/DEARM/AIRCHECK_mini_challenge/DevDataset_Aircheck.csv"
# Load the CSV file into a Pandas DataFrame
df_dev = Dataset(DevData_path).get_dataframe()
print("------------------------------------------------------------")
print("Number of binders in the dev set:", (df_dev["Label"] == 1).sum())
print("Number of non-binders in the dev set:", (df_dev["Label"] == 0).sum())
print(df_dev.head(3))

# Ensure that the file path is correct before running
TestData_path = "../datasets/DEARM/AIRCHECK_mini_challenge/TestDataset_Aircheck.csv"
df_test = Dataset(TestData_path).get_dataframe()
print("------------------------------------------------------------")
print("Number of compounds in the test set:", df_test.sum())
...
def train_model(
    CrossVal_data_train, CrossVal_data_test, CrossVal_label_train, CrossVal_label_test
):
    ...
    y_scores = model.predict_proba(CrossVal_data_test)[
        :, 1
    ...
    metrics = {
        ...
        "AUC-ROC": (
            roc_auc_score(CrossVal_label_test, y_scores)
            if len(set(CrossVal_label_test)) > 1
            else None
        ),
...
for fold_idx, (train_idx, test_idx) in enumerate(
    skf.split(TrainData_np, TrainLabel_np)
):
    CrossVal_data_train, CrossVal_data_test = (
        TrainData_np[train_idx],
        TrainData_np[test_idx],
    )
    CrossVal_label_train, CrossVal_label_test = (
        TrainLabel_np[train_idx],
        TrainLabel_np[test_idx],
    )
...
df_predictions_dev = pd.DataFrame(
    {
        ...
        "PredictedLabel": (predictions > Thr).astype(
            int
...
def compute_metrics(df_sorted, df_dev, top_n):
    ...
    tir = (
        hits / total_actual_positives if total_actual_positives > 0 else 0
...
# Print results
print(
    f"Precision at Top 100: {precision_100:.4f}, Hits: {hits_100}, TIR: {tir_100:.4f}"
)
...
prediction_df_sorted = df_predictions_test.sort_values(
