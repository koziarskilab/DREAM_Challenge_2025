...
def vina_docking(rigid_receptor, flex_receptor, lig_path):
    ...
    try:
        ...
        best_pdbqt_path = (
            VINA_DOCKING_RESULTS_PATH / f"{recep_bname}/{lig_bname}_out.pdbqt"
        )
        ...
        if POSE_FILTERING:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                all_cls = [
                    f for f in CRYSTAL_LIGANDS_DIR.iterdir() if f.suffix == ".sdf"
                ]
        ...
        if (best_sdf_path).exists():
            print(f"Docked ligand {lig_bname} with protein {recep_bname} successfully!")
