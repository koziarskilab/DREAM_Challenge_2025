#!/bin/bash
#SBATCH --job-name=DREAM_BASELINE_AUTOML_IMBALANCED_ATOMPAIR   # Updated job name
...
# Define the options for fps_type and loss_type
FPS_TYPES=("MACCS" "RDK" "AVALON" "ATOMPAIR")
# FPS_TYPES=("ATOMPAIR")  # Uncomment for single fingerprint testing

# Define imbalanced learning loss types (matching the Python script choices)
LOSS_TYPES=(
...
for fps_type in "${FPS_TYPES[@]}"; do
    for loss_type in "${LOSS_TYPES[@]}"; do
        ...
        LOG_DIR="${BASE_LOG_DIR}/${fps_type}_MLP_${loss_type}"
        mkdir -p ${LOG_DIR}

        # Run the Python script with the current combination
        echo "Running baseline_automl_imbalanced_learning.py with --fps_type=${fps_type}, --loss_type=${loss_type}"
